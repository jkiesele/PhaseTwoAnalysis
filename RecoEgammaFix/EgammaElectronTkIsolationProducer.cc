...
EgammaElectronTkIsolationProducer::EgammaElectronTkIsolationProducer(const edm::ParameterSet& config) : conf_(config)
{
  ...
  electronProducer_               = consumes<reco::GsfElectronCollection>(conf_.getParameter<edm::InputTag>("electronProducer"));
  
  trackProducer_           = consumes<reco::TrackCollection>(conf_.getParameter<edm::InputTag>("trackProducer"));
  beamspotProducer_        = consumes<reco::BeamSpot>(conf_.getParameter<edm::InputTag>("BeamspotProducer"));
...
void EgammaElectronTkIsolationProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  iEvent.getByToken(trackProducer_,tracks);
  ...
  iEvent.getByToken(beamspotProducer_,beamSpotH);
