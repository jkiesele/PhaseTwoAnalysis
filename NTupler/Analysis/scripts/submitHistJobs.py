...
parser.add_option('-s', '--selection',    dest='sel' , help='selection: 0 = no leptons, 1=e, 2=mu, 3=ee, 4=mumu'  , default=0,      type=int)
...
for myproc in processList :
    ...
    if opt.dojes :
        os.system('./bin/simplePlots %s %s %d 1'%(outDir,myproc,opt.sel))
    else:
        os.system('./bin/simplePlots %s %s %d'%(outDir,myproc,opt.sel))
